...
public:
	...
	int backcolorb,backcolorg,backcolorr;//标志背景颜色值，用于寻找轮廓
public:
	BOOL GetImageHSI(float &hue,float &satu,float &inten,C2DPoint *triPos);
	void FindImageContour(CArray<C2DPoint*,C2DPoint*> &p2DPoint);
	void SetPixel256(int nIndex,RGBQUAD cColor);
	RGBQUAD GetPixel256(int index);
	int GetPixelIndex(int x,int y);
	RGBQUAD GetPixel(LONG index);
	void SetPixel(LONG nIndex,COLORREF cColor);
...
private:
	...
	void ComputeMetrics();
public:
	bool CopyImageInMemory(CDib *refImg);
};
