...
#define TRIMIN(x,y,z) (x<MINT(y,z)?x:MINT(y,z))
 ...
 *   MW:已经验证，可以通过这个调用GetPixel(LONG index)以及SegPixel（LONG nindex,RGBQUAD cColor）
 ************************************************************************
 */ 
 ...
 ************************************************************************
 */
RGBQUAD CDib::GetPixel(int x, int y)//暂时先不用了，好像有问题
{
	// 颜色结构
	RGBQUAD cColor;

	if(x<0||x>=m_lpBMIH->biWidth||y<0||y>=m_lpBMIH->biHeight){
...
////MW:已经验证，可以先调用GetPixelOffset(int x, int y)获得存储位置。
void CDib::SetPixel(LONG nIndex, COLORREF cColor)//自己加上的，设置像素值
{
	int dimx=(m_lpBMIH->biWidth*m_lpBMIH->biBitCount+31)/32*4;
	int dimy=m_lpBMIH->biHeight;
	if(nIndex>=dimx*dimy||nIndex<0)
	{
		return;
	}
	m_lpImage[nIndex]=GetBValue(cColor);
	m_lpImage[nIndex + 1]=GetGValue(cColor);
	m_lpImage[nIndex + 2]=GetRValue(cColor);

}

//MW:已经验证，可以先调用GetPixelOffset(int x, int y)获得存储位置。
RGBQUAD CDib::GetPixel(LONG index)//适用于24位的bmp
{
	RGBQUAD cColor;
	int dimx=(m_lpBMIH->biWidth*m_lpBMIH->biBitCount+31)/32*4;
	int dimy=m_lpBMIH->biHeight;
	if(index>=dimx*dimy||index<0)
	{
		cColor.rgbBlue=0;
		cColor.rgbGreen=0;
		cColor.rgbRed=0;
	...
	{
	cColor.rgbRed   = m_lpImage[index+2];
	cColor.rgbGreen = m_lpImage[index + 1];
	cColor.rgbBlue  = m_lpImage[index];
	cColor.rgbReserved =0;  
	}
	return cColor;
...
int CDib::GetPixelIndex(int x, int y)
{
	int dimx=(m_lpBMIH->biWidth*m_lpBMIH->biBitCount+31)/32*4;
	int dimy=m_lpBMIH->biHeight;
	return (dimy-y-1)*dimx+x*m_lpBMIH->biBitCount/8;

}

RGBQUAD CDib::GetPixel256(int index) //貌似不对
{
	RGBQUAD cColor;
	cColor.rgbRed   = m_lpImage[index];
	cColor.rgbGreen = cColor.rgbRed;
	cColor.rgbBlue  = cColor.rgbRed;
	cColor.rgbReserved =0;   
	return cColor;

}

void CDib::SetPixel256(int nIndex, RGBQUAD cColor)
{
	m_lpImage[nIndex]=cColor.rgbBlue;
}

void CDib::FindImageContour(CArray<C2DPoint*, C2DPoint *> &p2DPoint)
{
	int i,j;
	RGBQUAD ccolor;

	for(i=1;i<m_lpBMIH->biHeight-1;i++){
		for(j=1;j<m_lpBMIH->biWidth-1;j++){
			int nIndex=GetPixelIndex(j,i);
			ccolor=GetPixel(nIndex);
			if(ccolor.rgbBlue!=backcolorb||ccolor.rgbGreen!=backcolorg||ccolor.rgbRed!=backcolorr){
				int n0=GetPixelIndex(j,i-1);
				int n1=GetPixelIndex(j,i+1);
				int n2=GetPixelIndex(j-1,i);
				int n3=GetPixelIndex(j+1,i);
				...
				BOOL b0,b1,b2,b3;
				int truenum=0;
				if(c0.rgbBlue!=backcolorb||c0.rgbGreen!=backcolorg||c0.rgbRed!=backcolorr)
				{
					b0=TRUE;
					truenum++;
				}
				if(c1.rgbBlue!=backcolorb||c1.rgbGreen!=backcolorg||c1.rgbRed!=backcolorr)
				{
					b1=TRUE;
					truenum++;
				}
				if(c2.rgbBlue!=backcolorb||c2.rgbGreen!=backcolorg||c2.rgbRed!=backcolorr)
				{
					b2=TRUE;
					truenum++;
				}
				if(c3.rgbBlue!=backcolorb||c3.rgbGreen!=backcolorg||c3.rgbRed!=backcolorr)
				{
					b3=TRUE;
					truenum++;
				}
				if(truenum<4)
				{
					...
					c->x=(float)j;
					c->y=(float)i;
					c->color.rgbRed=ccolor.rgbRed;
					c->color.rgbGreen=ccolor.rgbGreen;
					c->color.rgbBlue=ccolor.rgbBlue;
					c->color.rgbReserved=ccolor.rgbReserved;
					p2DPoint.Add(c);
				}
			}
		}
...
BOOL CDib::GetImageHSI(float &hue, float &satu, float &inten, C2DPoint *triPos)
{
	//提取纹理的HSI信息,如果triPos=NULL，则考虑全部象素，否则只考虑triPos内部的
	int i,j;
	hue=0.0f;
	satu=0.0f;
	inten=0.0f;
	...
	if(triPos==NULL)
	{
		for(i=0;i<m_lpBMIH->biWidth;i++)
		{
			...
			{
				int nIndex=GetPixelIndex(i,j);
				RGBQUAD cColor=GetPixel(nIndex);
				if((cColor.rgbBlue!=0&&cColor.rgbGreen!=0&&cColor.rgbRed!=0)&&((cColor.rgbBlue!=1&&cColor.rgbGreen!=1&&cColor.rgbRed!=1)))
				{
					float rt,gt,bt;
					rt=(float)cColor.rgbRed/255.0f;
					gt=(float)cColor.rgbGreen/255.0f;
					bt=(float)cColor.rgbBlue/255.0f;
					validnum++;
					float huet=1.0f/(float)cos((rt-gt+rt-bt)/(2*sqrt((rt-gt)*(rt-gt)+(rt-bt)*(gt-bt))));
					float intent=(float)(rt+gt+bt)/3.0f;
					float satut=1.0f-TRIMIN(rt,gt,bt)/intent;
					if(bt/intent>gt/intent)
						huet=2*3.14f-huet;
					huet/=2*3.14f;
					hue+=huet;
					inten+=intent;
...
//拷贝refImg中的数据到当前图像中,自己填写的，用于创建一个新的图像，并可以与write函数配合用
bool CDib::CopyImageInMemory(CDib *refImg)
{
	
	//只拷贝write函数中涉及的
	m_nColorTableEntries = refImg->m_nColorTableEntries;
	m_dwSizeImage = refImg->m_dwSizeImage;


	// 计算信息头和调色板的大小尺寸
	int nSizeHdr = sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * m_nColorTableEntries;
	m_lpBMIH = (LPBITMAPINFOHEADER) new char[nSizeHdr];
	memcpy(m_lpBMIH, refImg->m_lpBMIH,nSizeHdr);
	// 分配存放图象数据的内存
	m_lpImage = (LPBYTE) new char[m_dwSizeImage];
	memcpy(m_lpImage,refImg->m_lpImage,m_dwSizeImage);
	this->m_nBmihAlloc = this->m_nImageAlloc = crtAlloc; 
	return true;
